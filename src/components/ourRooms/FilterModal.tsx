    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-[#021521]/60 backdrop-blur-xs p-0 sm:p-4 font-figtree overflow-hidden">
      ...
      <div className="bg-white w-full max-w-2xl rounded-t-[24px] sm:rounded-[24px] shadow-2xl border border-slate-100 overflow-y-auto max-h-[90vh] sm:max-h-[85vh] p-5 sm:p-8 flex flex-col space-y-5 sm:space-y-6 relative animate-in fade-in zoom-in duration-200">
        ...
        <button
          type="button"
          onClick={onClose}
          className="absolute top-4 sm:top-6 right-4 sm:right-6 p-1.5 rounded-full text-slate-400 hover:text-slate-700 hover:bg-slate-100 transition-colors cursor-pointer"
        >
          <X className="w-5 h-5" />
        </button>
        ...
        <div className="pr-8 sm:pr-0">
          <h2 className="text-xl sm:text-2xl font-bold text-[#0B3558] tracking-tight">
            Filters
          </h2>
        ...
        <div>
          ...
          <p className="text-[11px] sm:text-xs text-slate-400 font-normal mb-5 sm:mb-6">
          ...
          <div className="relative pt-6 pb-2 px-1 dual-range-slider">
            ...
            >
              <span className="bg-[#DBEAFE] text-[11px] sm:text-xs font-bold px-3 py-0.5 sm:py-1 rounded-full shadow-xs whitespace-nowrap">
            ...
            {/* Boundary Labels */}
            <div className="flex justify-between text-[11px] sm:text-xs text-slate-500 font-medium mt-3">
              <span>₹5,000</span>
              <span>₹12,000</span>
            </div>
          </div>
            ...
            {amenityOptions.map((item) => {
              ...
              return (
                <button
                  ...
                  className={`relative flex flex-col items-center justify-center p-2 sm:p-3 rounded-2xl border transition-all cursor-pointer ${
                    isChecked
                ...
                >
                  <div
                    className={`absolute top-1.5 right-1.5 sm:top-2 sm:right-2 w-3 h-3 sm:w-3.5 sm:h-3.5 rounded-full border flex items-center justify-center transition-colors ${
                      isChecked
                  ...
                  <Icon
                    className={`w-3.5 h-3.5 sm:w-4 sm:h-4 mb-1 sm:mb-1.5 ${
                      isChecked ? "text-[#0B3558]" : "text-slate-600"
                    }`}
                  />
                  <span className="text-[9px] sm:text-[11px] font-medium text-center leading-tight">
                    {item.label}
                  </span>
                </button>
              );
        ...
        <div className="pt-3 sm:pt-4 border-t border-slate-100 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button
              type="button"
              onClick={onClose}
              className="px-4 sm:px-5 py-2 sm:py-2.5 rounded-full text-xs font-semibold text-slate-600 hover:bg-slate-100 border border-slate-200 transition-colors cursor-pointer"
            >
              Cancel
            </button>
            <button
              type="button"
              onClick={handleApply}
              className="bg-[#0B3558] hover:bg-[#082842] text-white px-5 sm:px-6 py-2 sm:py-2.5 rounded-full text-xs font-semibold shadow-xs transition-colors cursor-pointer"
            >
              Apply Filters
            </button>
          </div>
