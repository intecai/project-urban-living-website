...
const categoryTabs: { label: string; value: RoomCategory; icon: string }[] = [
  ...
  { label: "Double Rooms", value: "double", icon: "/images/rooms/DoubleRooms.png" },
  { label: "Triple Rooms", value: "triple", icon: "/images/rooms/TripleRooms.png" },
  { label: "Private Rooms", value: "private", icon: "/images/rooms/PrivateRooms.png" },
];
  ...
  return (
    <section className="w-full py-10 lg:py-14 font-figtree min-h-[600px]">
      <div className="max-w-[1440px] mx-auto px-4 sm:px-6 md:px-12 lg:px-16 space-y-8">
        ...
        {/* 1. Category Filter Tabs - Centered on Page */}
        <div className="flex items-center justify-center gap-3.5 sm:gap-4 overflow-x-auto pb-2 no-scrollbar w-full">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pt-2">
          ...
          <div>
            <h1 className="text-2xl sm:text-[28px] font-bold text-[#1F2937] font-jakarta tracking-tight">
              {sortedRooms.length} Rooms Available
            </h1>
            <p className="text-sm sm:text-base text-[#9CA3AF] font-jakarta font-normal mt-1">
              Across 6 Locations in Chennai
            </p>
          </div>

          {/* Right: Filter & Sort Controls */}
          <div className="flex items-center gap-4 sm:gap-5 self-start sm:self-auto font-jakarta">
            {/* Filter Button */}
            <button
              type="button"
              onClick={() => setIsFilterModalOpen(true)}
              className="flex items-center justify-center gap-2 px-5 py-2 h-[36px] bg-white border border-[#E2E8F0] rounded-[16px] text-sm font-medium text-[#1F2937] hover:bg-slate-50 hover:border-slate-300 transition-colors cursor-pointer"
            >
            ...
            <div className="flex items-center gap-2.5">
              ...
              <div className="relative">
                <select
                  ...
                  className="appearance-none bg-white border border-[#E2E8F0] rounded-[16px] pl-4 pr-9 py-2 h-[36px] text-sm font-medium text-[#1F2937] hover:bg-slate-50 hover:border-slate-300 transition-colors cursor-pointer focus:outline-hidden"
                >
        ...
        {paginatedRooms.length > 0 ? (
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-12 text-center space-y-3">
            <h3 className="text-lg font-bold text-[#011A2A]">No rooms match your filter criteria</h3>
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-center gap-6 sm:gap-8 pt-10 pb-6 font-jakarta select-none">
            {/* Previous Link */}
            <button
              type="button"
              disabled={currentPage === 1}
              onClick={() => setCurrentPage((prev) => Math.max(prev - 1, 1))}
              className="flex items-center gap-2 text-base sm:text-lg font-semibold text-[#4B5563] font-inter text-[14px] hover:text-[#4B5563] disabled:opacity-40 disabled:cursor-not-allowed transition-colors cursor-pointer"
            >
            ...
            <div className="flex items-center gap-4 sm:gap-6">
              {getPaginationItems(currentPage, totalPages).map((item, index) => {
                if (item === "...") {
                  return (
                    <span key={`ellipsis-${index}`} className="text-lg sm:text-xl font-normal text-[#9CA3AF] px-1">
                      ...
                    </span>
                  );
                }
                ...
                return (
                  <button
                    ...
                    className={`transition-all cursor-pointer ${
                      isActive
                        ? "w-11 h-11 rounded-full bg-[#FAC024] text-white font-bold text-lg sm:text-xl flex items-center justify-center shadow-xs"
                        : "text-lg sm:text-xl font-medium text-[#4B5563] hover:text-[#1F2937] px-2 py-1"
                    }`}
                  >
                    {page}
                  </button>
            ...
            <button
              ...
              className="flex items-center gap-2 text-base sm:text-lg font-semibold text-[#4B5563]font-inter text-[14px] hover:text-black disabled:opacity-40 disabled:cursor-not-allowed transition-colors cursor-pointer"
            >
              <span>Next</span>
